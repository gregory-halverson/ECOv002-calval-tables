...
def quick_look_plots_met(big_df_ss, time, out_path=None):
    ...
    fig.tight_layout()
    if out_path is None:
        # Default: present working directory, same filename as before
        out_path = os.path.join(os.getcwd(), f'auxiliary_eval_{time}.png')
    fig.savefig(out_path, dpi=300)

def quick_look_plots(big_df_ss, time, LE_var='LEcorr50', out_path=None):
    ...
    fig.tight_layout()
    if out_path is None:
        out_path = os.path.join(os.getcwd(), f'le_eval_{LE_var}_{time}.png')
